...
fn main() {
    ...
    // 配网成功后触发 MQTT 重连/重登录：blufi 事件线程在 `WifiConnected` 时置位「重连请求」标志，
    // mqtt 运行循环（run_session）据此丢弃当前会话、用新网络重连并 login（设计见详细设计 §5.5）。
    // 采用 `Arc<AtomicBool>` 而非 `tokio::sync::Notify` / `futures::channel::mpsc`：二者都依赖跨线程
    // waker，在 `futures::executor::block_on` 单线程模型下唤醒不可靠（实测配网触发后重连不生效，
    // 旧连接被永久阻塞在 read，直到 broker 端 RST 才被动断开）。
    // 主循环依靠 `runtime.run()` 自身每 ~500ms 的 tick 轮询该标志（无需跨线程唤醒），
    // 标志置位后至多一个 tick（500ms）内即中断旧会话、立即重连。
    let reconnect_req = Arc::new(AtomicBool::new(false));
    if cfg.blufi.enabled {
        ...
        // 给事件线程一份标志副本（move 进线程）。
        let reconnect_req_ev = reconnect_req.clone();
            ...
            for ev in blufi_ev_rx {
                match ev {
                    ...
                    BlufiEvent::WifiConnected { ssid } => {
                        ...
                        // 仅成功时触发：置位「重连请求」标志，主循环 tick 轮询到后即重连。
                        reconnect_req_ev.store(true, Ordering::SeqCst);
                    }
    ...
    futures::executor::block_on(mqtt_main(cfg, app_state, event_rx, cmd_tx, ui_state, client_id, ch, reconnect_req));
}
...
async fn mqtt_main(
    ...
    // 重连策略（最简二分）：
    //  - immediate=true（配网成功触发）：已切到新网络，立即重连并重新 login，无退避；
    //  - 其他（断网/错误）：退避 5s 后重连（原始行为，避免打爆 broker）。
    loop {
        let (immediate, reason) = run_session(&cfg, &app_state, &event_rx, &cmd_tx, &ui_state, client_id, ch, lwt_topic, &reconnect_req).await;
        if immediate {
...
async fn run_session(
    ...
    //  - immediate=false → 断网/错误，mqtt_main 退避后重连。
    // 进入新会话先消费上一次的重连请求标志，避免新会话一启动就立即被自己中断。
    reconnect_req.store(false, Ordering::SeqCst);
    ...
    // 配网成功后「立即重连」由 `reconnect_req` 标志驱动：runtime 在每 ~500ms 的 tick 循环顶部
    // 检查该标志（见 myrtio-mqtt `MqttRuntime::run_until`），置位即主动返回 `Ok(())`，本次
    // session 结束，mqtt_main 据 immediate 标志「立即」重连/login；断网/错误则 `Err`，退避。
    // 说明：不能用 `select(runtime.run(), 信号)`，因为 `StdTcpTransport::recv_timeout` 是同步阻塞
    // read，在 `futures::executor::block_on` 单线程下 `runtime.run()` 的 future 永不把 `Pending`
    // 交还外层 select，信号分支只会被 poll 一次（Notify/mpsc/AtomicBool 三次验证均失败）。
    let r = runtime.run_until(&**reconnect_req).await;
    match r {
        Ok(()) => {
            log::info!("mqtt: wifi provisioned — re-login immediately (no backoff)");
            (true, "reconnect requested".to_string())
        }
        Err(e) => (false, format!("runtime: {e:?}")),
    }
}
